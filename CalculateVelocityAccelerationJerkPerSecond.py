...
def calculate_jerk(csv_file):
    ...
    results = [average_velocity_magnitude, average_acceleration_magnitude, average_jerk_magnitude]
    return results
...
for i in range(len(subjectNumList)):
    data = [[], [], [], []]
    filename = root + "Sub" + str(subjectNumList[i]) + "/RightHapticDevice.csv"
    print(filename)
    appendList = calculate_jerk(filename)
...
# for i in range(1):
#     data = [[], [], [], []]
#     filename = root + "Sub" + str(i + 1) + "/LeftHapticDevice.csv"
#     print(filename)
#     row.append(calculate_jerk(filename))

with open("RHD_TrialResults/result.csv", 'w', newline='') as file:
